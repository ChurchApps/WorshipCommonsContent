...
import { songDirs, readJson, writeJson, readSongRaw, songJsonPath, orderSong } from "./lib.mjs";
...
for (const { langDir, folder, dir } of songDirs(ROOT)) {
  ...
  // Raw file only — do not stamp inherited parent themes onto a translation.
  ...
  if (!dryRun) { song.themes = after; writeJson(file, orderSong(song)); }
}
