...
How the song folders in this repo are built. Overview for GitHub visitors: [README.md](README.md).
...
| `composition/chart.pdf` | Chord chart in the original key: Courier when the text encodes as WinAnsi, else embedded Noto Sans Mono (Cyrillic, Latin Extended), else Malayalam shaped with HarfBuzz in Noto Sans Malayalam, chords placed over the shaped syllable. Other keys are transposed on demand. Chinese is skipped (no CJK font vendored) |
